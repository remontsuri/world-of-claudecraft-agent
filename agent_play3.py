...
class Game:
    ...
    def fwd(self):
        self.send("Runtime.evaluate",{"expression":"window.__game.controller.move({forward:true})"})
    def stop(self):
    ...
    def shot(self,path):
        ...
        open(path,'wb').write(__import__('base64').b64decode(r.get('result',{}).get('data','')))
...
def run(seconds=240):
    g=Game(); t0=time.time(); step=0; ax=ay=None
    while time.time()-t0<seconds:
        st=json.loads(g.state()); step+=1
        if ax is None: ax,ay=st['px'],st['py']
        mb=nearest(st)
        if mb:
            ...
            # patrol around anchor in a radius ~25, sweep angle slowly
            ang=math.atan2(ay-st['py'],ax-st['px'])+(step*0.05)%(2*math.pi)
            # if drifted too far, head back to anchor
            if math.hypot(st['px']-ax,st['py']-ay)>35:
                ang=math.atan2(ay-st['py'],ax-st['px'])
            g.face(ang); g.fwd()
            if step%12==0: g.stop()
        time.sleep(0.3)
    g.stop(); g.shot('cdp_play3.png')
