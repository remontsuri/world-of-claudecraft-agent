...
## `fly-woc/*.md` — документация (13 файлов)

| Файл | Строк | О чём |
|---|---|---|
| `README.md` | 279 | что это за линия, как запускать, где данные и как читать отчёты |
| `ARCHITECTURE.md` | 134 | устройство: схема, входы/выходы, читауты, потоки |
| `AUDIT-2026-09-17.md` | 176 | аудит линии: блокер GPU-обучения, ускорения горячего пути, потоки для сред |
| `CYBORG.md` | 363 | «как всунуть муху в игру»: обзор решений, киборг (LLM-кортекс), двусторонняя связка «LLM в мозгу», научная база и контроль FLM |
| `DEPENDENCIES.md` | 62 | зависимости и версии, что нужно для прогонов |
| `ECOSYSTEM.md` | 123 | внешние проекты и лицензии, что берём и что закрыто |
| `ENV_ROBUST.md` | 86 | устойчивость к падению сервера игры в долгих прогонах |
| `FILES.md` | 77 | карта файлов (этот файл) |
| `GITHUB-AUDIT.md` | 215 | аудит репозитория: ветки, история, вес дерева и .git, утечки секретов, дубликаты коннектомов |
| `GOOGLE-FLY.md` | 145 | статья Google про муху: где опубликована (Cell + препринт), числа, датасет CC-BY и связь с нашими 8 835 нейронами |
| `GPU-DEVICE.md` | 112 | выбор устройства (ROCm/CUDA/CPU), что заведомо не поломка |
| `ROADMAP.md` | 64 | фазы F0–F11 с критериями приёмки |
| `VISUAL.md` | 152 | визуализация: Neuroglancer, deep-links по bodyId, вьюеры MaleCNS |
