...
class Game:
    ...
    def move(self): self.send("Runtime.evaluate",{"expression":"window.__game.controller.move({forward:true})"})
...
GOALS = [("Sableweb",-60,4), ("CopperDig",-84,-64), ("WolfRun",-2,70)]
def run(seconds=400):
    ...
    while time.time()-t0<seconds:
        ...
        if mb:
            ...
            if mb['dist']>1.2:
                if abs(((ang - getattr(g,'_last_ang',ang)+math.pi)%(2*math.pi))-math.pi) > 0.3:
                    g.face(ang); g._last_ang=ang
                g.move()
            else:
                g.stop(); g.target(mb['id']); g.attack()
                if step%4==0: g.interact()
            sys.stdout.write(f"\r[{int(time.time()-t0)}s] FIGHT {mb['name']} d={mb['dist']:.1f} hp={st['hp']} lvl={st['lvl']} xp={st['xp']}")
            sys.stdout.flush()
        else:
            gx,gy=None,None
            for name,gx0,gy0 in GOALS:
                if math.hypot(st['px']-gx0,st['py']-gy0)>22: gx,gy=gx0,gy0; break
            if gx is None: gx,gy=-60,4
            d_goal=math.hypot(st['px']-gx,st['py']-gy)
            if d_goal>10:
                ang=math.atan2(gy-st['py'],gx-st['px'])
            else:
                ang=(step*0.15)%(2*math.pi)
            # face only if heading changed a lot; otherwise just move (face resets move)
            if abs(((ang - getattr(g,'_last_ang',ang)+math.pi)%(2*math.pi))-math.pi) > 0.3:
                g.face(ang); g._last_ang=ang
            g.move()
            if step%20==0:
                sys.stdout.write(f"\r[{int(time.time()-t0)}s] ->({gx},{gy}) d={d_goal:.0f} hp={st['hp']} lvl={st['lvl']} xp={st['xp']}")
                sys.stdout.flush()
        time.sleep(0.07)
