...
(async()=>{
  ...
  // blur any focused element, focus body
  await page.evaluate(()=>{ if(document.activeElement&&document.activeElement.blur)document.activeElement.blur(); window.focus(); });
  await sleep(150);
  const focus=await page.evaluate(()=>({active:document.activeElement?.tagName}));
  ...
  console.log(JSON.stringify({pos0,pos1,dist:d,focusAfterBlur:focus}));
  await b.disconnect();
