...
Зависимости — инструменты разработки (`typescript`, `esbuild`, `vitest`, `@types/node`)
и **ровно одна рантайм-зависимость**: `ws` (+ `@types/ws`) для WS-транспорта живого мира (задача A1).
Причина (решение 2026-09-18, по факту из upstream): в Node 20 глобального `WebSocket` нет,
а сама игра зависит от `ws@^8.21.0` и свой Node-бот пишет именно так — `bot/gateway.ts`
импортирует `ws` и инжектирует `socketFactory: (url) => new WebSocket(url)`, в тестах заглушка,
`tsconfig.bot.json` = `types: ["node"]`, lib ES2022, без DOM. Наш `src/bridge/ws_socket.ts`
повторяет этот рисунок и разрешает сокет в объявленном порядке: инжектированная фабрика →
глобальный `WebSocket` (Node 22+, ноль зависимостей) → `ws` → **громкий ПРОВАЛ** с двумя
лекарствами (тихий фолбэк запрещён). В бандле `ws` внешний (`--external:ws`), поэтому его
отсутствие не ломает сборку стенда и бриджа.
...
4. ❌ Переписывать факты игры руками в нашем коде (только импорт + `verify_facts`);
   ❌ держать в репозитории копию дерева игры (`.game-cjs/` и подобное) и подменять
   импорт алиасами на неё: факты берутся из исходников upstream, а копия с
   переименованными расширениями ломает типы и уводит сверку фактов с upstream
