...
def nearest_hostile(st):
    best=None; bd=1e9
    for e in st['ents']:
        if e.get('hostile') and e['dist']<bd:
            bd=e['dist']; best=e
    return best

def run(seconds=120):
    g=Game(); t0=time.time(); step=0
    # head to forest: bias movement toward +x / -y (away from town center which is ~0,1.5)
    while time.time()-t0 < seconds:
        ...
        if mb:
            ...
            # wander toward forest edge: aim generally south-east, sweep
            ang = (math.pi*0.15) + 0.4*math.sin(step*0.1)
            g.move_to(ang)
            if step%10==0: g.stop()
        time.sleep(0.35)
    g.stop(); g.shot('cdp_play2.png')
