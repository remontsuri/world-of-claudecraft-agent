...
from safety import should_force_heal


def test_should_force_heal():
    # module-level helper must exist and trigger at critical hp
    assert should_force_heal({"hp_frac": 0.03}) is True, "must force heal at 3% hp"
    assert should_force_heal({"hp_frac": 0.19}) is True, "must force heal < 0.2"
    assert should_force_heal({"hp_frac": 0.20}) is False, "0.20 is NOT critical"
    assert should_force_heal({"hp_frac": 0.50}) is False, "half hp is not critical"
    assert should_force_heal({"hp_frac": 1.0}) is False, "full hp is not critical"
