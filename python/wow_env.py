...
class WoWClassicEnv(gym.Env):
    ...
    # ---- capability API commands (bridge to IWorld* methods) ----
    def sell_junk(self):
        res = self._request({"cmd": "sell_junk"})
        return res.get("info", {})

    def harvest_node(self, node_id: str, confirm_effect_use: bool = False):
        res = self._request({"cmd": "harvest_node", "node_id": node_id, "confirm_effect_use": confirm_effect_use})
        return res.get("info", {})

    def craft_item(self, recipe_id: str, commission: bool = False, count: int = 1):
        res = self._request({"cmd": "craft_item", "recipe_id": recipe_id, "commission": commission, "count": count})
        return res.get("info", {})

    def accept_quest(self, quest_id: str, selection: str | None = None):
        req = {"cmd": "accept_quest", "quest_id": quest_id}
        if selection:
            req["selection"] = selection
        res = self._request(req)
        return res.get("info", {})

    def turn_in_quest(self, quest_id: str):
        res = self._request({"cmd": "turn_in_quest", "quest_id": quest_id})
        return res.get("info", {})

    def loot_corpse(self, corpse_id: int):
        res = self._request({"cmd": "loot_corpse", "corpse_id": corpse_id})
        return res.get("info", {})
