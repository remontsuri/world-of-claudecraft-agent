...
def verify_heal(c):
    ...
    if h1 > h0 or h1 >= _player_hp_max(c['after']):
        return 'success'
    # Was a potion (or any heal item) actually consumed? If something was used
    # but HP did not rise, treat as inconclusive — regen may still be ticking,
    # or the heal was partially resisted. Never reward, but don't punish either.
    import re
    global _POTION_RE
    ...
    if p1c < p0:
        return 'inconclusive'   # potion spent but hp did not rise (regen ticking?) — rare
    # Nothing consumed AND HP unchanged → no-op (no supplies). MUST be 'failure':
    # 'inconclusive' gave ~zero negative signal, so the agent kept re-trying heal
    # at crit HP instead of retreating (observed death loop, run 14844).
    return 'failure'
