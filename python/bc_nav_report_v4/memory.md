...
- PPO FINE-TUNE PREP (2026-08-13): написан train_ppo_from_b1.py (read-only-safe:
  inspect + parity + smoke) + diag_parity.py. АУДИТ ВЫЯВИЛ ФАТАЛЬНЫЙ БЛОКЕР:
  BC-B1 обучен на 568-dim (567 raw + in_combat_range), SB3 MlpPolicy ждёт
  ровно observation_space.shape -> при WrapperEnv(568) policy_net.0=(512,568),
  но при прямом CurriculumEnv(567) =(512,567). Паритет FAIL: argmax 75.3%
  (должно быть ~100% при точном копировании весов) -> СТОП, PPO НЕ запущен.
  КОРНЕВАЯ ПРИЧИНА: размерность входа 567 vs 568 несовместима; прямой маппинг
  первого слоя (512,568)->(512,567) невозможен. НУЖНО РЕШЕНИЕ: либо (а) заставить
  SB3 принять 568 (WrapperEnv + убедиться что SB3 build берёт 568 из obs_space),
  либо (б) переобучить BC-B1 на 567 (потеря рабочего latch-признака).
  РЕШЕНИЕ НЕ ПРИНЯТО вслепую — ждать OK юзера. ДОЛГИЕ PPO-ПРОГОНЫ ЗАПРЕЩЕНЫ.
