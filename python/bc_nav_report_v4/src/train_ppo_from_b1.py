...
class WrapperEnv(gym.Env):
    """Appends in_combat_range (computed from 567 obs) -> obs becomes 568.

    Does NOT modify Sim/obs.ts. Only exposes the derived feature B1 used.
    """
    ...
    def reset(self, **kw):
        ...
        icr = bool(t["has"] and t["dist"] <= nf.MELEE_YD)
        return np.concatenate([o, [1.0 if icr else 0.0]]).astype(np.float32), info

    def step(self, action):
        o, r, term, trunc, info = self.base.step(action)
        t, m = nf.decode_nav_obs(o, ABILITY_SLOTS)
        icr = bool(t["has"] and t["dist"] <= nf.MELEE_YD)
        return np.concatenate([o, [1.0 if icr else 0.0]]).astype(np.float32), \
            r, term, trunc, info
...
def parity_test(sb3_policy, bc_model, n_states=1000, seeds=(42, 43, 44)):
    ...
    for obs in obs_buf:
        ...
        with torch.no_grad():
            ...
            # SB3 net expects 568 (567 raw + in_combat_range) — same as WrapperEnv.
            tgt, _ = nf.decode_nav_obs(obs, ABILITY_SLOTS)
            icr = 1.0 if (tgt["has"] and tgt["dist"] <= nf.MELEE_YD) else 0.0
            xt568 = torch.from_numpy(
                np.concatenate([obs, [icr]]).astype(np.float32)).unsqueeze(0).to(device)
            sb3_latent = sb3_policy.mlp_extractor(
                sb3_policy.extract_features(xt568))
            sb3_logit = sb3_policy.action_net(sb3_latent[0]).cpu().numpy()
...
def eval_sb3(model, seeds):
    ek = deaths = 0
    for seed in seeds:
        env = WrapperEnv(ce.CurriculumEnv(stage=3, player_class="warrior",
                                          max_steps=400, frame_skip=5))
        o, _ = env.reset(seed=seed)
        kills = dead = 0
        for _ in range(1, 401):
...
def main():
    ...
    base = lambda: WrapperEnv(ce.CurriculumEnv(stage=3, player_class="warrior",
                                               max_steps=400, frame_skip=5))
