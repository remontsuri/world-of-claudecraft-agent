...
# Lazy-load PPO model (only when needed)
_ppo_model = None
...
def get_ppo_model():
    ...
    if _ppo_model is None:
        ...
        model_path = Path('D:/world-of-claudecraft/data/ppo_fly_offline.zip')
        if model_path.exists():
            _ppo_model = PPO.load(model_path, device='cpu')
        else:
            print(f'[env] PPO model not found at {model_path}, using random actions')
    return _ppo_model
...
class WoCFlyBrainEnv(gym.Env):
    ...
    def step(self, action=None):
        """Execute one step with PPO control + brain simulation."""
        ...
        if action is None:
            ...
            if ppo is not None:
                ...
                action = int(action)
            elif self.use_brain and self.brain is not None:
                # Fallback to WTA brain control
                ...
                brain_rates[0, :min(len(rates), self.brain.n_neurons)] = rates_tensor[:min(len(rates), self.brain.n_neurons)]
                spikes = self.brain.step(brain_rates, n_steps=10)
                action = self.motor.decode(spikes[0])
            else:
                action = 0  # default forward
        
        # Run brain simulation in background (for logging/visualization)
        if self.use_brain and self.brain is not None:
            rates = self.sensor.encode(obs, max_rate=100.0)
            rates_tensor = torch.tensor(rates, dtype=torch.float32, device=self.device).unsqueeze(0)
            brain_rates = torch.zeros(1, self.brain.n_neurons, device=self.device)
            brain_rates[0, :min(len(rates), self.brain.n_neurons)] = rates_tensor[0, :min(len(rates), self.brain.n_neurons)]
            _ = self.brain.step(brain_rates, n_steps=10)
