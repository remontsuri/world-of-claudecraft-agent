...
(async () => {
  ...
  const r = await page.evaluate(() => {
    ...
    try {
      // click-to-move: set clickMoveGoal to a point 10 units ahead
      const tx = p.pos.x + 10, tz = p.pos.z;
      if (typeof g.input.clickMoveGoal === 'function') { g.input.clickMoveGoal(tx, tz); return 'clickMoveGoal(fn)'; }
      g.input.clickMoveGoal = {x:tx, z:tz}; return 'clickMoveGoal(set)';
    } catch(e){ return 'ERR:'+e.message; }
  });
  await new Promise(r=>setTimeout(r,3000));
  const after = await page.evaluate(() => { const p=window.__game.sim.player; return {x:p.pos.x,z:p.pos.z}; });
  console.log('clickMove result:', r, '| moved', Math.hypot(after.x-before.x, after.z-before.z).toFixed(2));
