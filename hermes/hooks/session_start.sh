...
add "ГЛАВНОЕ: автономный Java-бот (woof-agent/) проходит квестовую цепочку в WoC через мост к игре и доказывает это живым прогоном: [Agent] SUMMARY с quests_done ≥ 1."
add "Инварианты: игру и её исходники не трогаем; браузер через CDP не перезагружаем; hermes-agent/hermes-gateway не убиваем; force-push запрещён; молчаливых фолбэков нет; пороги метрик не двигаем после замера; archive/ заморожен — не удаляем и не переписываем."
