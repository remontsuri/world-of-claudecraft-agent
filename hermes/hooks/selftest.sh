...
expect_block "удаление из архива заблокировано"        '{"event":"pre_tool","command":"rm archive/fly-line/fly-woc/data/quest_oracle_214.json"}'
expect_block "git rm в архиве заблокирован"            '{"event":"pre_tool","command":"git rm -r archive/fly-line/reference"}'
expect_block "перенос из архива заблокирован"          '{"event":"pre_tool","command":"git mv archive/fly-line/fly-woc fly-woc"}'
expect_block "полные данные MaleCNS заблокированы"      '{"event":"pre_tool","command":"gsutil cp gs://flyem-male-cns/v1.0/connectome-data/flat-connectome/ ."}'
expect_block "feather-веса заблокированы"              '{"event":"pre_tool","command":"git add data/annotations.feather"}'
