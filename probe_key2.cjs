...
(async()=>{
  ...
  // click canvas to focus game
  await page.evaluate(()=>{ const c=document.querySelector('canvas'); if(c){ c.focus(); c.click(); } });
  await sleep(200);
  // check what has focus
  const focus=await page.evaluate(()=>({active:document.activeElement?.tagName, isInput:document.activeElement?.tagName==='INPUT'||document.activeElement?.tagName==='TEXTAREA'}));
  ...
  console.log(JSON.stringify({pos0,pos1,dist:d,focus}));
  await b.disconnect();
