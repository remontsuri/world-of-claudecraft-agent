...
## Раскладка obs: индексы выводятся, а не зашиты

Игра считает размер вектора как `63 + 2*ABILITY_SLOTS + 2*КВЕСТЫ`
(`src/sim/obs.ts: obsSize()`), а `ABILITY_SLOTS` — это максимум по классам, то есть
величина, которая меняется от версии к версии. Любой зашитый индекс после блока
способностей при этом сдвигается и читает чужие поля.

| Сборка | obs | действий | способностей | цель | мобы | интеракт | квесты |
|---|---:|---:|---:|---:|---:|---:|---:|
| текущий upstream (main и все теги v0.39.2–v0.42.2) | 607 | 61 | 48 | 112 | 121 | 151 | 156 |
| сборка с 28 слотами (проверено) | 567 | 41 | 28 | 72 | 81 | 111 | 116 |

`obs_layout.py` выводит все границы из длины obs и числа квестов (+ сверяет с
числом действий, если оно известно), `python3 obs_layout.py` — самопроверка,
`tools/test_obs_layout.py` — обе раскладки разом и регрессия на 607.
`quest_oracle.py` и `fly_brain.py` (v1 и v2) больше не содержат индексных
констант. Что это лечит на 567-сборке: `quest_slots` падал с
`IndexError: index 602 is out of bounds for axis 0 with size 567`, а v2-энкодер
— с `ValueError: cannot reshape array of size 411 into shape (2)`; «интеракт»
читался из полей квестов.

Важное следствие: **веса привязаны к числу действий**. Чекпойнт, обученный на
61 действии, не загружается в сборку с 41 (`n_actions` в чекпойнте — 61), то есть
на другой раскладке обучение нужно повторять, переносить можно только базовые
13 действий и совпадающие слоты способностей.
