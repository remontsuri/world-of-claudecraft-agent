...
Таблиц в репозитории три, по одной на сборку: `data/quest_oracle_204.json`
(v0.36–v0.39.0), `data/quest_oracle_214.json` (v0.40.x) и `data/quest_oracle.json`
(224, main/v0.42.x). Порядок квестов в obs — это `QUEST_ORDER` конкретной сборки,
поэтому таблица от другой версии даёт чужие координаты: у 224-й с 214-й он
расходится уже со второй позиции (204-я — префикс 214-й, это везение, а не правило).

Таблицу генерируют из игрового кода своей сборки:

```bash
bash tools/make_quest_oracle.sh v0.40.0 data/quest_oracle_214.json
# или взять готовую: WOC_QUEST_TABLE=data/quest_oracle_214.json
```

Скрипт тянет разреженный клон (только `src/sim`: полный tarball игры ~780 МБ) и
проверяет арифметику `obs = 63 + 2*способности + 2*квесты` по метаданным.
`quest_oracle.py --check` сверит числа, а `tools/test_quest_oracle.py` — схему,
координаты и `world_bounds` (у v0.40.0 `maxZ=2420`, у v0.42.2 `2380` — границы
тоже разъезжаются). Чтение слотов из чужой таблицы теперь **отказ**, а не
предупреждение.
