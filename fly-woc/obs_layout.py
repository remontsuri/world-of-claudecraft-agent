...
def detect(obs_size: int, n_actions: int | None = None, n_quests: int | None = None,
    ...
    if n_actions is not None:
        ...
        if n_quests is not None and int(n_quests) != derived:
            # Самый частый случай на практике: таблица оракула снята с другой
            # версии игры. Игра отдаёт свою величину — ей и верим, а таблицу
            # нужно перегенерировать под свою сборку.
            raise ValueError(
                f"таблица квестов не от этой сборки: в таблице {int(n_quests)} квестов, "
                f"а окружение говорит про {derived} (obs={obs_size}, действий={n_actions}, "
                f"способностей={ability_slots}, хвост паладина={tail}). Квестовый блок в obs "
                f"будет стоять на {FIXED_PREFIX + ABILITY_STRIDE * ability_slots}, и слоты "
                f"разъедутся. Перегенерируй таблицу под свою версию: "
                f"tools/dump_quest_oracle.ts (см. README) или укажи путь в WOC_QUEST_TABLE.")
        n_quests = derived
    else:
